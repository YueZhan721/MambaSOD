...
from models.MambaSOD_model import PDFNet
...
num_parms = 0
for p in model.parameters():  # calculate the model parameter
    num_parms += p.numel()
...
def train(train_loader, model, optimizer, epoch,save_path):
    ...
    try:
        for i, (images, gts, depths) in enumerate(train_loader, start=1):
            ...
            s1, s2, s3, s4, s5= model(images, depths)
            loss1 = CE(s1, gts)
            loss2 = CE(s2, gts)
            loss3 = CE(s3, gts)
            loss4 = CE(s4, gts)
        ...
        if (epoch) % 10 == 0:
            torch.save(model.state_dict(), save_path+'PDFNet_epoch_{}.pth'.format(epoch))
    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), save_path+'PDFNet_epoch_{}.pth'.format(epoch+1))
    ...
    with torch.no_grad():
        ...
        if epoch==1:
            ...
            if mae<best_mae:
                best_mae=mae
                best_epoch=epoch
                torch.save(model.state_dict(), save_path+'MambaSOD_epoch_best.pth')
