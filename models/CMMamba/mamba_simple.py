...
class Mamba(nn.Module):
    def __init__(
        ...
        self.in_proj = nn.Linear(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs)  # 对应图中的MLP
        ...
        self.A_log = nn.Parameter(A_log)  # nn.Parameter()将一个张量自动注册为nn.Module的参数，从而在训练过程中可以被优化器更新
        ...
        if bimamba_type == "v1":
            ...
            self.in_proj_extra_c = nn.Linear(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs)

        self.out_proj = nn.Linear(self.d_inner, self.d_model, bias=bias, **factory_kwargs)

    def forward(self, hidden_states, inference_params=None, extra_emb=None, concated = None):  # hidden_states=extra_emb: (1,16384,32)
        """
        hidden_states: (B, L, D)
        ...
        if extra_emb is None:
            ...
            xz = rearrange(
                ...
                "d (b l) -> b d l",
                l=seqlen,
            )  # @ 为矩阵乘法运算符. hidden_states: (b,16384,32)->(32,16384*b)--> todo 获得 xz: (b,128,16384)
            if self.in_proj.bias is not None:
                xz = xz + rearrange(self.in_proj.bias.to(dtype=xz.dtype), "d -> d 1")
            extra_emb = rearrange(
                ...
                "d (b l) -> b d l",
                l=seqlen,
            )  # extra_emb(Pan): (b,16384,32)->(32,16384*b)-->(b,128,16384)
            if self.in_proj_extra.bias is not None:
                extra_emb = extra_emb + rearrange(self.in_proj_extra.bias.to(dtype=xz.dtype), "d -> d 1")
            concated = rearrange(
                self.in_proj_extra_c.weight @ rearrange(concated, "b l d -> d (b l)"),
                "d (b l) -> b d l",
                l=seqlen,
            )  # extra_emb(Pan): (b,16384,32)->(32,16384*b)-->(b,128,16384)
            if self.in_proj_extra_c.bias is not None:
                concated = concated + rearrange(self.in_proj_extra_c.bias.to(dtype=xz.dtype), "d -> d 1")
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            ...
            #         out = F.linear(rearrange(out + out_b, "b d l -> b l d") / 2, self.out_proj.weight, self.out_proj.bias)
                ...
                # F.linear(rearrange(out_z, "b d l -> b l d"), out_proj_weight, out_proj_bias)
                if not self.if_devide_out:
                    out = F.linear(rearrange(out*out_c + out_b*out_c, "b d l -> b l d"), self.out_proj.weight, self.out_proj.bias)
                else:
                    out = F.linear(rearrange(out*out_c + out_b*out_c, "b d l -> b l d") / 2, self.out_proj.weight, self.out_proj.bias)
