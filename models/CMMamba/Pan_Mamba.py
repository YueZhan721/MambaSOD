...
class SingleMambaBlock(nn.Module):
    ...
    def forward(self, ipt):  # ipt(1, 16384, 32)
        x, residual = ipt
        residual = x+residual
...
class TokenSwapMamba(nn.Module):
    ...
    def forward(self, ms, pan, ms_residual, pan_residual):
        ...
        B, N, C = ms.shape
        ms_first_half = ms[:, :, :C//2]  # ms_first_half (1, 16384, 16)
        pan_first_half = pan[:, :, :C//2]  # pan_first_half (1, 16384, 16)
        ms_swap = torch.cat([pan_first_half, ms[:, :, C//2:]], dim=2)  # ms_swap (1, 16384, 32)
        pan_swap = torch.cat([ms_first_half, pan[:, :, C//2:]], dim=2)  # pan_swap (1, 16384, 32)
        ms_swap = self.msencoder(ms_swap)  # ms_swap (1, 16384, 32)
        pan_swap = self.panencoder(pan_swap)  # pan_swap (1, 16384, 32)
...
class CrossMamba(nn.Module):
    ...
    def forward(self, ms, ms_resi, pan):  # ms (1,16384,32), pan (1,16384,32), ms_resi==0
        ms_resi = ms+ms_resi  # ms_resi (1,16384,32)
        ...
        ms = global_f.transpose(1, 2).view(B, C, 128, 128)  # Reshape: (1,16384,32)-->(1, 128,128, 32)
        ms = (self.dwconv(ms)+ms).flatten(2).transpose(1, 2)  # (1, 128,128, 32)-->(1,16384,32)
        return ms, ms_resi
...
class Net(nn.Module):
    ...
    def forward(self, ms, _, pan):  # ms(1,4,32,32), pan(1,1,128,128)
        ...
        ms_f = self.ms_to_token(ms_f)  # (1,32,128,128)-->(1,128*128=16384,32)
        pan_f = self.pan_to_token(pan_f)  # (1,32,128,128)-->(1,16384,32)
        residual_ms_f = 0
        residual_pan_f = 0
        ms_f, residual_ms_f = self.ms_feature_extraction([ms_f, residual_ms_f])  # ms_f(1,16384,32)
        pan_f, residual_pan_f = self.pan_feature_extraction([pan_f, residual_pan_f])  # pan_f(1,16384,32)
        ms_f, pan_f, residual_ms_f, residual_pan_f = self.swap_mamba1(ms_f, pan_f, residual_ms_f, residual_pan_f)
        ms_f, pan_f, residual_ms_f, residual_pan_f = self.swap_mamba2(ms_f, pan_f, residual_ms_f, residual_pan_f)  # ms_f(1,16384,32) pan_f(1,16384,32)

        ms_f = self.patchunembe(ms_f, (h, w))  # ms_f (1,16384,32)--> (1,32,128, 128), 这里由三维转四维是为了接下来的浅层融合
        pan_f = self.patchunembe(pan_f, (h, w))  # pan_f (1,16384,32)--> (1,32,128, 128)
        ms_f = self.shallow_fusion1(torch.concat([ms_f, pan_f], dim=1))+ms_f  # ms_f (1,32,128, 128)
        pan_f = self.shallow_fusion2(torch.concat([pan_f, ms_f], dim=1))+pan_f  # pan_f (1,32,128, 128)
...
if __name__ == '__main__':
    ...
    # 实例化模型得到分类结果
    lms_image = torch.randn(1, 4, 32, 32).to(device)
    bms_image = torch.randn(1, 4, 32, 32).to(device)
    pan_image = torch.randn(1, 1, 128, 128).to(device)
